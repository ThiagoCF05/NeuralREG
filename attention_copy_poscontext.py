...
class Attention:
    ...
    def decode(self, pos_encoded, entity_encoded, refex, entity, entity_tokens):
        refex = list(refex)
        ...
        for word in refex:
            ...
            vector = dy.concatenate([attention_pos, attention_entity, last_output_embeddings, entity_type_embedding, entity_gender_embedding])
            s = s.add_input(vector)
    ...
    def generate(self, pos_context, entity, entity_tokens):
    ...
    def beam_search(self, pos_context, entity, entity_tokens):
        embedded = self.embed_sentence(pos_context)
        pos_encoded = self.encode_sentence(self.encpos_fwd_lstm, self.encpos_bwd_lstm, embedded)
    ...
    def get_loss(self, pos_context, refex, entity, entity_tokens):
        embedded = self.embed_sentence(pos_context)
        pos_encoded = self.encode_sentence(self.encpos_fwd_lstm, self.encpos_bwd_lstm, embedded)
        ...
        return self.decode(pos_encoded, entity_encoded, refex, entity, entity_tokens)
    ...
    def validate(self):
        ...
        for i, devinst in enumerate(self.devset):
            ...
            if self.config.beam == 1:
                outputs = [self.generate(pos_context, entity, entity_tokens)]
            else:
                outputs = self.beam_search(pos_context, entity, entity_tokens)
    ...
    def train(self):
        ...
        for epoch in range(self.config.epochs):
            ...
            for i, traininst in enumerate(self.trainset):
                ...
                loss = self.get_loss(pos_context, refex, entity, entity_tokens)
                losses.append(loss)
