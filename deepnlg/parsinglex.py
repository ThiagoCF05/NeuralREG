...
def parse(in_file):
    ...
    for entry in entries:
        ...
        for lex in lexEntries:
            ...
            try:
                ...
                for snt in otripleset:
                    ...
                    for otriple in snt:
                        ...
                        orderedtripleset_snt.append(
                            Triple(subject=e1.replace('\'', ''), predicate=pred, object=e2.replace('\'', '')))
            ...
            lexList.append(
                Lex(comment=comment, lid=lid, text=text, template=template, orderedtripleset=orderedtripleset,
...
def generate(entryset, in_file, out_file, lng):
    ...
    for entry_xml in entries:
        ...
        entry = list(filter(lambda entry: entry.eid == eid and entry.size == str(size) and entry.category == category,
                            entryset))[0]
